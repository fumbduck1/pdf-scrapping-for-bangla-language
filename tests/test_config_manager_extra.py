...
class TestConfigManagerExtra(unittest.TestCase):
    ...
    def test_missing_pdf_triggers_error(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            cfg = create_job_config(pdf_path=str(Path(tmp) / "missing.pdf"), output_root=str(output_dir))
            errors = get_config_manager().validate_config(cfg)
            self.assertTrue(any("pdf file not found" in e.lower() for e in errors))
    ...
    def test_output_dir_created_when_missing(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            cfg = create_job_config(pdf_path=str(pdf_file), output_root=str(output_dir))
            errors = get_config_manager().validate_config(cfg)
